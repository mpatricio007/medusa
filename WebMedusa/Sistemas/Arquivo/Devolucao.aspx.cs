...
{
    public partial class Devolucao : PageCrud<EmprestimoVolumeBLL>
    {
        ...
        {            
            base.SetAdd();
            cTextoUsuDevolucao.Focus();
        }

        protected override void Get()
        {
            this.cTextoUsuDevolucao.Focus();
            this.cTextoUsuDevolucao.Text = String.Empty;
            this.cTextoUsuDevolucao_TextChanged(null, null);
        ...
        protected override void btCancelar_Click(object sender, EventArgs e)
        {
            cTextoUsuDevolucao.Focus();
            this.cTextoUsuDevolucao.Text = String.Empty;            
            cTextoUsuDevolucao_TextChanged(null, null);
            listaVolumes = new List<Medusa.DAL.Volume>();
            gridVolumes.DataBind();            
        }

        protected void cTextoUsuDevolucao_TextChanged(object sender, EventArgs e)
        {   
            UsuarioFuspBLL usu = new UsuarioFuspBLL();
            usu.GetUsuarioFuspPorCpf(this.cTextoUsuDevolucao.Text);
        ...
        protected void txtVolume_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                if (ObjBLL.VolumeEstaEmprestado(this.txtVolume.Text))
                {
                    ...
                    Util.ShowMessage("este volume não está emprestado!");
        ...
        {  
            if (ObjBLL.DevolucaoVolumes(listaVolumes, this.cTextoUsuDevolucao.Text,this.txtObservacao.Text))
                msg(String.Format("devolução de {0} volume(s) para {1} efetuado com sucesso!",listaVolumes.Count(),this.txtNome.Text));
            else
                msgError("erro!");
            Get();
            this.cTextoUsuDevolucao.Focus();
