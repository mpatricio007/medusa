...
namespace Medusa.Sistemas.Bolsa
{
    public partial class Bolsas : PageCrud<BolsaBLL>
    {
        ...
        {
            ObjBLL.Get(PkValue);
            this.txtCodigo.Text = Convert.ToString(ObjBLL.ObjEF.id_bolsa);
            this.cTextoNome.Text = ObjBLL.ObjEF.nome;
            this.cTextoSigla.Text = ObjBLL.ObjEF.sigla;
            this.cTextoHoras.Text = Convert.ToString(ObjBLL.ObjEF.num_horas);
            this.ckStatus.Checked = ObjBLL.ObjEF.status;
        }

        protected override void Set()
        {
            ObjBLL.ObjEF.id_bolsa = Convert.ToInt32(this.txtCodigo.Text);
            ObjBLL.ObjEF.nome = this.cTextoNome.Text;
            ObjBLL.ObjEF.sigla = this.cTextoSigla.Text;
            ObjBLL.ObjEF.num_horas = this.cTextoHoras.Value.GetValueOrDefault();
