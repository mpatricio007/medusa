...
{
    public partial class ControleMaterial : System.Web.UI.UserControl
    {
        public List<RequisicaoMaterial> Value
        {
            get
            {
                if (Session[ID] == null)
                    Session[ID] = new List<RequisicaoMaterial>();
                return (List<RequisicaoMaterial>)Session[ID];
            }
            set
            {
                Session[ID] = value;
        ...
        protected void Set()
        {
            var req = new RequisicaoMaterial();
        ...
        protected void msgFalta(string msg)
        {
            var matBLL = new MaterialConsumoBLL();
            lblMsg.BackColor = System.Drawing.Color.Red;
            lblMsg.ForeColor = System.Drawing.Color.White;
            lblMsg.Text = string.Format("Quantidade indisponível no momento! Quantidade em estoque é de {0}, favor solicitar a compra deste matérial!", matBLL.ObjEF.qtde_total);
        }
