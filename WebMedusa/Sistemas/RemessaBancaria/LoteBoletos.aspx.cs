...
{
    public partial class LoteBoletos : PageCrud<LoteBoletoBLL>
    {
        ...
        {  
            ...
            if (!IsPostBack)
            {
                btExcluirPagamento.OnClientClick = "return confirm('confirma o estorno do boleto?')";
                btExcluirPagamento0.OnClientClick = "return confirm('confirma o estorno do boleto?')";
        ...
        protected void SetPagamento(RemessaTit rem)
        {
            ...
            rem.valor = cValor.Value.GetValueOrDefault();
            rem.id_projeto = this.cDdlProjeto1.Id_projeto;
            rem.descricao = this.cTextoDescricao.Text;
            rem.Boleto = new CodigoBarrasBoleto(this.txtBoleto.Text);
            rem.dataVencto = this.cDataVencto.Value.GetValueOrDefault();
            rem.id_banco_destino = this.cDdlBancos1.Id_banco;            
        }

        protected void GetPagamento(int intId_remessa)
        {
            
            RemessaTitBLL remBLL = new RemessaTitBLL();
            ...
            //this.lbLote.Text = Convert.ToString(remBLL.ObjEF.id_lote);
            this.cTextoCedente.Text = remBLL.ObjEF.nome_fav_cedente;          
            this.cValor.Value = remBLL.ObjEF.valor;
            ...
            this.txtBoleto.Text = remBLL.ObjEF.Boleto.RepresentacaoNumerica;
            this.cDataVencto.Value = remBLL.ObjEF.dataVencto;
            this.cDdlBancos1.Id_banco = remBLL.ObjEF.id_banco_destino;
        ...
        protected void SetGridPagamentos()
        {
            ...
            gridPagamentos.DataSource = ObjBLL.ObjEF.Boletos.Where(it => it.nome_fav_cedente.Contains(this.txtNomeFavorecido.Text)).ToList();
        ...
        protected bool validaBoleto(out string msg)
        {
            ...
            rem.ObjEF.id_remessa = Convert.ToInt32(txtId_pagamento.Text);
            if (!rem.Exists())
            {
                if (rem.ObjEF.Boleto.ValidaCodBarra())
                {
                    this.cDataVencto.Value = boleto.DataVencimento();
                    //this.cDdlBancos1.Id_banco = boleto.Id_banco();
                    this.cDdlBancos1.Id_banco = BancoBLL.RetornaIdpeloCodigo(boleto.CodigoBanco());
                    this.cValor.Value = boleto.Valor();
        ...
        protected void btRejeitar_Click(object sender, EventArgs e)
        {
            if (Convert.ToBoolean(rbRejeicao.SelectedValue))
            {
                ...
                if (rem.Rejeitar(cTextoMotivo.Text, ref msg))
                {
                    ...
                    msgPagtoError(msg);
                cTextoMotivo.Text = String.Empty;
            }
