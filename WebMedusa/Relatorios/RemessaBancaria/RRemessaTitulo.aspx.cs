...
{
    public partial class RRemessaTitulo : BasePage
    {
        protected void btGerarRelatorio_Click(object sender, EventArgs e)
        {
            Cache.Remove("lotes");
            dRelatorio.InnerHtml = String.Format("<iframe src=\"../../Relatorios/RemessaBancaria/ReportRemessaTitulo.aspx?pk1={0}&pk2={1}&pk3={2}\" width=\"100%\" height=\"1000px\"></iframe>", Convert.ToString(cInteiroDe.Value.GetValueOrDefault()).Criptografar(),
               Convert.ToString(cInteiroAte.Value.GetValueOrDefault()).Criptografar(),Util.DateToString(cData1.Value).Criptografar());
            dRelatorio.DataBind();
        ...
        protected void btGerarArquivo_Click(object sender, EventArgs e)
         {
             ...
             tbReenvio.Visible = ((List<LoteBoleto>)Cache["lotes"]).Count() > 0;
        ...
        public void SetGridLotesEnviados()
        {
            ...
            gridLotesEnviados.DataSource = (List<LoteBoleto>)Cache["lotes"];
        ...
        protected void btOk_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (var item in (List<LoteBoleto>)Cache["lotes"])
                {
                    var lBLL = new LoteBoletoBLL();
        ...
        protected void gridLotesEnviados_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            Cache["lotes"] = ((List<LoteBoleto>)Cache["lotes"]).Where(it => it.id_lote != Id_lote).ToList();
            if(((List<LoteBoleto>)Cache["lotes"]).Count() > 0)
                SetGridLotesEnviados();
            else
                Util.ChamarScript("closeLotesDialog();", "");
            e.Cancel = true;
