...
{
    public partial class RRemessaCons : BasePage
    {
        protected void btGerarRelatorio_Click(object sender, EventArgs e)
        {
            Cache.Remove("lotes");
            dRelatorio.InnerHtml = String.Format("<iframe src=\"../../Relatorios/RemessaBancaria/ReportRemessaCons.aspx?pk1={0}&pk2={1}&pk3={2}\" width=\"100%\" height=\"1000px\"></iframe>", Convert.ToString(cInteiroDe.Value.GetValueOrDefault()).Criptografar(),
        ...
        protected void btGerarArquivo_Click(object sender, EventArgs e)
        {
            ...
            tbReenvio.Visible = ((List<LoteCons>)Cache["lotes"]).Count() > 0;
        ...
        public void SetGridLotesEnviados()
        {
            ...
            gridLotesEnviados.DataSource = (List<LoteCons>)Cache["lotes"];
        ...
        protected void btOk_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (var item in (List<LoteCons>)Cache["lotes"])
                {
                    var lBLL = new LoteConsBLL();
        ...
        protected void gridLotesEnviados_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            Cache["lotes"] = ((List<LoteCons>)Cache["lotes"]).Where(it => it.id_lote != Id_lote).ToList();
            if (((List<LoteCons>)Cache["lotes"]).Count() > 0)
                SetGridLotesEnviados();
            else
                Util.ChamarScript("closeLotesDialog();", "");
            e.Cancel = true;
