...
{
    public partial class ListaEmails : System.Web.UI.UserControl
    {
        public List<DAL.Email> Value
        {
            get
            {
                if (ViewState[ID] == null)
                    Value = new List<DAL.Email>();
                return (List<DAL.Email>)ViewState[ID];
            }
            set
            {
                ViewState[ID] = value;
        ...
        private void set()
        {
            var em = index < 0 ? new DAL.Email() : Value[index];
            em = this.cEmail1.Value;
            if (index < 0)
                Value.Add(em);
            else
                Value[index] = em;
